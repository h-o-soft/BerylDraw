...
import { getCell, type CellUpdate } from "../../domain/grid";
import { PaintCellsCommand } from "../commands/paint-cells-command";
import type { Tool, ToolEvent, ToolContext } from "./tool";

export class ColorPenTool implements Tool {
  readonly name = "colorpen";
  readonly icon = "C";
  readonly shortcut = "C";
  ...
  private addPoint(event: ToolEvent, context: ToolContext): void {
    ...
    const existing = getCell(context.grid, event.cellX, event.cellY);
    const cell: Cell = {
      char: existing.char,
